...
X3.to_csv('ImputedAll.tsv',sep="\t",index=False)
...
rfecv = rfecv.fit(X4train, ym)

print('Optimal number of features :', rfecv.n_features_)
print('Best features :', X4train.columns[rfecv.support_])

featureSel = [f for f,s in zip(X4train.columns, rfecv.support_) if s]
print featureSel
...
result = bayes_cv_tuner.fit(X4train[featureSel].values, ym, callback=status_print)
...
clf2.fit(X4train[featureSel],ym)
pd.Series(clf2.feature_importances_,index=list(X4train[featureSel].columns.values)).sort_values(ascending=True).head(50).plot(kind='barh',figsize=(12,18),title='XGBOOST FEATURE IMPORTANCE')

feati=gene+'_feat.png'
plt.savefig(feati)

mcpred=clf2.predict(X4train[featureSel])
...
clf2.fit(X4train[featureSel],ym)
pred=clf2.predict(X4test[featureSel])
proba=clf2.predict_proba(X4test[featureSel])

testset['Direction']=pred
testset['P_Direction']=proba[:,0]

clf2.fit(X4train[featureSel],conf)
predc=clf2.predict(X4test[featureSel])
probac=clf2.predict_proba(X4test[featureSel])
